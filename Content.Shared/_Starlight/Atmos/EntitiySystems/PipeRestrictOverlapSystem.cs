...
public sealed class PipeRestrictOverlapSystem : EntitySystem
{
    ...
    {
        SubscribeLocalEvent<PipeRestrictOverlapRPDComponent, AnchorStateChangedEvent>(OnAnchorStateChanged);
        SubscribeLocalEvent<PipeRestrictOverlapRPDComponent, AnchorAttemptEvent>(OnAnchorAttempt);

        _nodeContainerQuery = GetEntityQuery<NodeContainerComponent>();
    }

    private void OnAnchorStateChanged(Entity<PipeRestrictOverlapRPDComponent> ent, ref AnchorStateChangedEvent args)
    {
        if (!args.Anchored)
            return;
    ...
    private void OnAnchorAttempt(Entity<PipeRestrictOverlapRPDComponent> ent, ref AnchorAttemptEvent args)
    {
        if (args.Cancelled)
            return;
